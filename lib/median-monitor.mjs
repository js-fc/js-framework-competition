...
class MedianMonitor extends LitElement {
	...
	ping() {
        ...
        if (this.bucket.length > MedianMonitor.bucketSize) {
			this.bucket.shift();
        }

		let a = this.bucket.slice();
		a.sort();
        this.rate = a[~~(a.length/2)];
        this.lastTime = stop;
	}
...
customElements.define("median-monitor", MedianMonitor);